...
1. **PLANNING** — Collect requirements. Produce a plan for the implementation. Meet these, then hand back to the user to advance:
   - plan-written: The plan is written into the plan artifact.
2. **ITERATING** — Implement the plan. Implement any additional user requests or feedback. Implement any review comments the user has approved for implementation. Meet these, then hand back to the user to advance:
...
3. **REVIEW** — Wait for review or approval of the PR. Meet these, then hand back to the user to advance:
   - pr-reviewed: The PR has been reviewed.
4. **MERGING** — Add the PR to the merge queue. If the PR exits the merge queue, re-add it. Meet these, then advance it yourself:
