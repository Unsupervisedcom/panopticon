...
The golden behavioral spec for the local-git flow — the collapsed graph
(`PLANNING → ITERATING → COMPLETE`, no REVIEW, no MERGING), both foreground states
user-advanced, responsibility gating at each stage, the iterate-back free move, the
inability to skip straight to COMPLETE, no forge dependencies, and the universal drop.
"""
...
def test_transition_graph_is_the_happy_path_plus_drop() -> None:
    # No REVIEW, no MERGING — ITERATING advances straight to COMPLETE.
    assert set(WF.transitions("PLANNING")) == {"ITERATING", "DROPPED"}
    assert set(WF.transitions("ITERATING")) == {"COMPLETE", "DROPPED"}
    assert list(WF.transitions("COMPLETE")) == []
    assert "REVIEW" not in set(WF.labels())
    assert "MERGING" not in set(WF.labels())


def test_both_foreground_states_are_user_advanced() -> None:
    # No background agent-driven state: both foreground states are user-gated.
...
def test_no_forge_skills() -> None:
    assert WF.skills() == ()  # open-pr / babysit-ci / babysit-merge are forge-only
...
def test_core_operations_per_state() -> None:
    ...
    assert WF.operations("ITERATING") == {"advance": "COMPLETE", "drop": "DROPPED"}
...
def test_full_lifecycle_planning_to_complete() -> None:
    task = WF.start_task("t1", "r1", at="t0")
    for nxt in ("ITERATING", "COMPLETE"):
        _advance(task, nxt)
    assert task.state == "COMPLETE"
    assert [h.to_state for h in task.history] == ["PLANNING", "ITERATING", "COMPLETE"]
...
def test_cannot_advance_from_iterating_with_unresolved_responsibilities() -> None:
    ...
    with pytest.raises(ResponsibilitiesNotMet):
        WF.apply_transition(task, "COMPLETE", at="t2")  # committed etc. still PENDING
...
def test_partial_resolution_still_gates() -> None:
    ...
    with pytest.raises(ResponsibilitiesNotMet):
        WF.apply_transition(task, "COMPLETE", at="t2")  # committed + others still PENDING
...
def test_drop_is_allowed_from_every_state_and_bypasses_gating() -> None:
    for start in ("PLANNING", "ITERATING"):
        task = WF.start_task("t1", "r1", at="t0")
        if start == "ITERATING":
            _advance(task, "ITERATING")
