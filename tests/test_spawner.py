...
class _FakeRunner:
    ...
    def spawn(self, task_id: str, *, env_file: str | None = None, workspace: str | None = None, image: str | None = None, docker_in_docker: bool = False, memo: str | None = None, initial_prompt: str | None = None, progress: Callable[[LifecyclePhase], None] | None = None) -> str:
        self.spawned.append({"task_id": task_id, "env_file": env_file, "workspace": workspace, "image": image, "docker_in_docker": docker_in_docker, "memo": memo, "initial_prompt": initial_prompt})
...
def test_heal_caps_respawns_then_surfaces_a_crash_looping_task() -> None:
    ...
    spawner = Spawner(
        ...
        now=lambda: clock["t"], max_respawns=3, respawn_reset=60.0,
    )
    for _ in range(6):
...
def test_heal_resets_the_respawn_budget_after_a_survivor_window() -> None:
    ...
    spawner = Spawner(
        ...
        cache=CloneCache("/cache", run=_no_op_run, exists=lambda _p: True), tasks_root="/tasks",
        git=GitClones(run=_no_op_run), images=_FakeImageBuilder(),  # type: ignore[arg-type]
        now=lambda: clock["t"], max_respawns=2, respawn_reset=60.0,
    )
