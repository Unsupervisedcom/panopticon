"""The agent-CLI adapter seam (ADR 0014): the registry, and the claude adapter reproducing today's
rendered surface (argv, MCP config, overview, trust, model tier, hook-payload parsing) byte-for-byte.
No LLM — the real CLI exec (:meth:`ClaudeAgentCLI.launch`) is never called here."""
...
def test_launch_argv_appends_interrupt_prompt_on_respawn_for_agent_turn(tmp_path: Path) -> None:
    ...
    project.mkdir(parents=True)
    (project / "session.jsonl").write_text("{}")
    from panopticon.container.agent_cli import INTERRUPT_PROMPT
...
# -- claude: MCP config + workflow overview -----------------------------------------------------
...
# -- claude: hook payload seam (background-task gating) ------------------------------------------


def test_read_hook_payload_tolerates_empty_and_invalid(tmp_path: Path) -> None:
    cli = ClaudeAgentCLI()
