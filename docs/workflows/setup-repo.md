...
**When to use:** run a repo's host-side setup in a shell — today, minting a Claude auth
token via `claude setup-token`. Attach to complete the interactive flow, and the token
lands in the repo's env-file.
