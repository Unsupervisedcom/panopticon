...
|---|---|---|
| **PLANNING** | The agent collects requirements and writes a `plan.md` artifact (press `a` to read it) plus a token estimate. | **You** — approve the plan with `/advance`. |
| **ITERATING** | The agent implements the plan, commits and pushes, opens a **draft PR**, and gets CI green. | **You** — advance once you're happy the change is ready for review. |
| **REVIEW** | The task waits for the PR to be reviewed/approved by a peer. | **You** — advance once the PR is reviewed. |
| **MERGING** | The agent adds the PR to the merge queue and re-adds it if it falls out. | **The agent** — it advances itself once the PR is merged. |
| **COMPLETE** | Terminal. The change has landed. | — |
