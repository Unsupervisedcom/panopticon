...
- **The dashboard stays fast and trustworthy.** It's plain, deterministic software reading
  task state — no model in the loop to stall or surprise you.
...
| **Runner** | The per-machine worker. For each task it spawns a container, injects that repo's secrets, and creates the task's branch. One runner per machine — this is what lets a fleet span machines. | Tasks moving `queued → … → live` |
...
5. **The agent works.** It writes code on its branch, runs tests, opens a PR, and shepherds CI —
   all inside its container, reporting progress back to the control plane. It handles the steps a
