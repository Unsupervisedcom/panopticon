...
class Spawner:
    ...
    def _report(self, task_id: str, phase: LifecyclePhase, detail: str | None = None) -> None:
        ...
        if phase == LifecyclePhase.STARTING:
            ...
            _log.error("task %s: failed — %s", task_id, detail)  # a spawn step, an exit, a crash loop
        try:
    ...
    def reconcile(self, task: JsonObj) -> None:
        ...
        reason = self._runner.exit_reason(task["id"])
        if reason is not None:
            self._report(task["id"], LifecyclePhase.FAILED, detail=reason)  # composes `failed` + why
        elif status in _IN_PROGRESS:
            self._client.clear_lifecycle(task["id"])  # container gone without a trace → composes `down`
