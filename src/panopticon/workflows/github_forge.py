    ...
    def skills(self) -> Sequence[Skill]:
        ...
        return (
            ...
            Skill(
                ...
                "## State file\n\n"
                "Persist cross-turn state in `.panopticon-babysit-merge-state.json` at the repo "
                "root (gitignored — add to `.gitignore` if absent):\n"
                "```json\n"
                ...
                "delete the state file; stop.\n"
                "- **`CLOSED`** (not merged) → the PR was closed without merging. Do NOT advance. "
                "Delete the state file, leave a clear message for the user, and stop "
                ...
                "the state file. If `requeue_count` > 5, bail to the user and stop.\n\n"
                ...
                "```\n"
                "Record the returned Bash task ID as `watch_bash_id` in the state file. "
                "End the turn (the watcher runs in the background).\n\n"
                "## Re-invocation (state file exists from a prior turn)\n\n"
                "1. Read the state file. If `started_at` is more than 2h ago → bail to the user, "
                ...
                "4. Clear `watch_bash_id` in the state file (`null`).\n"
