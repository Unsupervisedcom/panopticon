...
class Workflow(ABC):
    ...
    #: tasks (and discover repos/workflows) through the control plane. The orchestration MCP
    #: tools are gated to workflows that opt in (the task service checks this flag for the
    #: acting task); default off, so an ordinary workflow's agent can mutate only tasks it
    #: already knows, never create them.
